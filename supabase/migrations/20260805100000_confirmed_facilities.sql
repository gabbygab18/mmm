-- "Confirmed" is a second, separate gate from "approved": approved means the
-- profile looks complete/legitimate; confirmed means admin has additionally
-- verified the facility directly (e.g. a phone call) before it's discoverable
-- by musicians. A facility can be approved but not yet confirmed.
...
-- Musician-side discovery must only ever surface confirmed facilities.
