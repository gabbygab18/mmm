-- Fix: Prevent duplicate alerts when initiating a request with initial proposal
-- Only alert on proposal_suggested if request is past 'initiated' status
-- (initial proposal is part of request creation, already alerted via request_initiated)
...
BEGIN
  ...
  -- Skip alert if this is the initial proposal for a request still in 'initiated' status
  -- (the request_initiated trigger already notified the other party)
  IF request_row.status = 'initiated' THEN
    RETURN NEW;
  END IF;
