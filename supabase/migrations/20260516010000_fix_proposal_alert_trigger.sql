-- Fix: Use request status instead of time-based check to prevent duplicate alerts
-- Only alert on proposal_suggested if this is NOT the initial proposal
-- (check: if there are other proposals for this request, alert; if none, skip)
