...
/** Larger than any real point-to-point distance within the continental US —
    passed as the radius boost so every approved facility shows up here
    regardless of the musician's own travel-radius setting, not just ones
    within it. */
const SHOW_ALL_RADIUS_MILES = 5000
...
export default async function MusicianDiscoverPage() {
  ...
  const { data: centers, error } = await supabase.rpc('get_nearby_centers_for_musician_with_expansion', {
    result_limit: 500,
    radius_boost_miles: SHOW_ALL_RADIUS_MILES,
  })
  ...
  return (
    <section className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6 font-poppins">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="font-garamond text-[28px] font-bold text-ocean-900">Browse Facilities</h1>
          <p className="mt-1 text-sm text-ocean-900/70">Every approved, profile-complete memory care community, nearest first.</p>
          {centers && centers.length > 0 && <p className="mt-2 text-xs text-ocean-900/60">Showing {centers.length} result{centers.length !== 1 ? 's' : ''}</p>}
        </div>
      </div>
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-3">
                    <Link
                      ...
                      className="mr-2 inline-block rounded-lg border border-ocean-300 px-3 py-1.5 text-xs font-semibold text-ocean-900 transition hover:bg-ocean-100"
                    >
