              ...
              <div className="flex gap-4 p-4">
                {musician.profile_image_url ? (
                  <img
                    ...
                    className="h-20 w-20 flex-shrink-0 rounded-full border border-ocean-200/70 object-cover"
                  />
                ) : (
                  <div className="flex h-20 w-20 flex-shrink-0 items-center justify-center rounded-full border border-ocean-200/70 bg-ocean-100 text-xl font-semibold text-ocean-700">
                    {musician.musician_name.charAt(0).toUpperCase()}
                  </div>
                )}
                <div className="min-w-0">
                  <p className="text-lg font-semibold text-ocean-900">{musician.musician_name}</p>
                  <p className="text-sm text-ocean-900/70">ZIP {musician.musician_zip_code}</p>
                  <p className="mt-1 text-xs font-semibold uppercase tracking-wide text-ocean-700">{formatDistance(musician.distance_miles)}</p>
                  <p className="mt-2 text-sm text-ocean-900">
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    <Link
                      ...
                      className="inline-block rounded-lg border border-ocean-300 px-3 py-1.5 text-xs font-semibold text-ocean-900 transition hover:bg-ocean-100"
                    >
                      View full profile
                    </Link>
                    <Link
                      href={`/dashboard/requests/new?centerLocationId=${id}&musicianId=${musician.musician_id}`}
                      className="inline-block rounded-lg border border-ocean-300 bg-ocean-50 px-3 py-1.5 text-xs font-semibold text-ocean-700 transition hover:bg-ocean-100"
                    >
                      ...
                      <button
                        ...
                        className={`flex h-8 w-8 items-center justify-center rounded-lg border transition ${
                          favoritedIds.has(musician.musician_id)
                            ...
                            : 'border-ocean-300 text-ocean-900/40 hover:text-ocean-700'
                        }`}
                      >
