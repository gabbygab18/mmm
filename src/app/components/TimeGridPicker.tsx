...
const ACCENT_STYLES: Record<Accent, { selected: string; anchor: string; hover: string }> = {
  ...
  brand: {
    selected: 'bg-ocean-800/85 border-ocean-800',
    anchor: 'ring-2 ring-ocean-400 ring-offset-1',
...
export function TimeGridPicker({
  ...
  return (
    <div className="rounded-xl border border-ocean-200/70 bg-ocean-50 p-3 font-poppins">
      <div className="mb-2 flex items-center justify-between gap-2">
        <p className="text-xs font-semibold uppercase tracking-wide text-ocean-900/60">Time grid picker</p>
        <p className="text-xs text-ocean-900/70">
          {formatTimeLabel(toTimeString(activeRange.start))} - {formatTimeLabel(toTimeString(activeRange.end))}
        </p>
      </div>

      <div className="max-h-[360px] overflow-y-auto rounded-lg border border-ocean-200/70 bg-white p-2">
            ...
            return (
              <button
                ...
                className={[
                  ...
                  inRange ? `${styles.selected} text-white` : `border-ocean-200/70 bg-white text-ocean-900 ${styles.hover}`,
      ...
      <p className="mt-2 text-[11px] text-ocean-900/60">Drag across blocks to select a continuous window in 30-minute increments.</p>
    </div>
  )
}
